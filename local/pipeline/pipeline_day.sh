...
"$PY" local/pipeline/collect_dataset.py --every "$EVERY" --until "$UNTIL" 2>&1 | grep -v '^WARNING'
...
sleep 5
"$PY" local/pipeline/train_model.py --epochs "$EPOCHS" --imgsz 960 --patience 12 2>&1 | grep -vE '^WARNING|^\s*$'
STATUS=${PIPESTATUS[0]}
