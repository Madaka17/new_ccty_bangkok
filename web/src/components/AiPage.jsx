...
function Bubble({ role, text, mode }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className={`flex ${me ? 'justify-end' : 'justify-start'}`}>
      {!me && (
        <span className="w-8 h-8 rounded-full bg-gold-50 border border-gold-100 flex items-center justify-center mr-2 shrink-0 mt-1">
          <SparkleIcon className="w-4 h-4" />
        </span>
      )}
      <div className={`max-w-[85%] rounded-3xl px-4 py-3 text-[15px] leading-relaxed whitespace-pre-wrap ${me ? 'bg-lavender-600 text-white rounded-br-lg' : 'bg-white/85 border border-cream-200 text-ink-900 rounded-bl-lg'}`}>
        {text}
...
export default function AiPage({ active, cameras, camid, onPickCamera, onToast, pendingQuestion, onQuestionConsumed }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-4 lg:h-[calc(100vh-11rem)] min-h-[560px]">
      {/* Chat */}
      <section className="glass rounded-[2rem] flex flex-col overflow-hidden" aria-label="แชทกับผู้ช่วยการจราจร">
        <div className="px-5 pt-5 pb-3 flex items-start gap-3">
          ...
          {summary?.ready && (
            <span className="hidden sm:inline-flex items-center gap-1.5 rounded-full bg-sage-50 border border-sage-100 px-3 py-1.5 text-xs text-sage-700">
        ...
        <div ref={listRef} className="flex-1 overflow-y-auto scroll-soft px-5 py-2 space-y-3 min-h-[280px]">
          ...
          {busy && (
            <div className="flex items-center gap-2 text-sm text-ink-600 pl-10">
              ...
              <span className="w-2 h-2 rounded-full bg-gold-400 animate-bounce [animation-delay:120ms]" />
              <span className="w-2 h-2 rounded-full bg-gold-400 animate-bounce [animation-delay:240ms]" />
              กำลังดูเส้นจราจรให้...
            </div>
          )}
        </div>
        ...
        <div className="px-5 pb-5 pt-2">
          <div className="flex flex-wrap gap-1.5 mb-2">
            {SUGGESTIONS.map((s) => (
              <button key={s} type="button" onClick={() => ask(s)} disabled={busy} className="cursor-pointer rounded-full bg-lavender-50 text-lavender-700 border border-lavender-100 px-3 py-1.5 text-xs hover:bg-lavender-100 transition-colors duration-200 disabled:opacity-50">
                {s}
              </button>
            ))}
          </div>
          <form
            ...
            className="flex items-center gap-2 rounded-full bg-white/85 border border-cream-200 pl-5 pr-1.5 py-1.5 focus-within:border-lavender-400 transition-colors duration-200"
          >
            ...
            <motion.button type="submit" whileTap={{ scale: 0.95 }} disabled={busy || !input.trim()} className="cursor-pointer rounded-full bg-gold-400 text-ink-900 px-5 py-2.5 text-sm font-semibold hover:bg-gold-200 transition-colors duration-200 disabled:opacity-50">
              ถาม
            </motion.button>
          </form>
        </div>
      ...
      <aside className="flex flex-col gap-3 lg:overflow-y-auto scroll-soft">
          ...
          {!summary?.ready ? (
            ...
            <ul className="space-y-1.5">
              {summary.congested.map((r) => (
                <li key={r.name}>
                  <button type="button" onClick={() => ask(`${r.name} ตอนนี้ระบายรถเป็นยังไง`)} className="cursor-pointer w-full text-left flex items-center gap-2 rounded-2xl px-2 py-1.5 hover:bg-cream-100 transition-colors duration-200">
        ...
        <div className="glass rounded-3xl p-4">
          ...
          <select id="ai-cam" value={camid || ''} onChange={(e) => onPickCamera(e.target.value)} className="cursor-pointer w-full rounded-full bg-white/85 border border-cream-200 px-3 py-2 text-xs text-ink-900 outline-none focus:border-lavender-400">
            {cameras.map((c) => (
          ...
          <div className="mt-2 relative rounded-2xl overflow-hidden bg-cream-100 aspect-video">
            ...
            {!live && (
              <div className="absolute inset-0 flex items-center justify-center text-xs text-ink-600">
                <span className="w-6 h-6 rounded-full border-4 border-gold-100 border-t-gold-400 animate-spin mr-2" />
                กำลังมองดูถนน...
              </div>
            )}
          </div>
          <div className="mt-2 grid grid-cols-3 gap-1.5">
            {[
              ...
              <div key={i} className="rounded-2xl bg-white/70 border border-cream-200 px-2 py-1.5 flex items-center gap-1.5">
                ...
                <div className="mt-3 flex gap-1">
                  {[1, 3, 5, 10].map((v) => (
                    <button
                      ...
                      aria-pressed={fps === v}
                      className={`cursor-pointer flex-1 rounded-full py-1 text-xs transition-colors duration-200 ${fps === v ? 'bg-lavender-600 text-white' : 'bg-lavender-50 text-lavender-700'}`}
                    >
                      {v} FPS
                    </button>
                  ))}
          ...
          <button type="button" onClick={captureView} className="cursor-pointer mt-3 w-full rounded-full bg-gold-400 text-ink-900 py-2 text-sm font-semibold hover:bg-gold-200 transition-colors duration-200">
            บันทึกวิวของฉัน
          </button>
        </div>
      </aside>
